...
class Level1 extends Screen {
	...
	private IntegerProperty shotCount = new SimpleIntegerProperty(SHOT_LIMIT);
	private Stage stage;
	...
	private void updateFrame() {
		// check if the game isn't over so it doesn't keep running after it's
		// over
		if (gameOn) {
			goNextLevel();
			goGameOver();
	...
	/*
	 * 2 labels in the game- makeTimer and makeCounter. makeLabel helps these
	 * methods
	 */
	 ...
	 * 
	 * @param character
	 *            = things the projectiles are colliding with
	 * @param projectiles
	 *            = all the projectiles you want to check
	 * @return
	 */
